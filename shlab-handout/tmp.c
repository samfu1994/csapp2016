...
void prog_exec(char * , char ** argv, int);
...
void eval(char *cmdline)
{
    ...
    if(! (type = builtin_cmd(argv) )){
      sigprocmask(SIG_BLOCK, &mask_one, &prev);
      if((pid = Fork()) == 0){
        //child
        sigprocmask(SIG_SETMASK, &prev, NULL);
        if (sigprocmask(SIG_UNBLOCK, &prev, NULL) == -1)
      ...
      sigprocmask(SIG_SETMASK, &prev, NULL);
      if(!bg){
        printf("it is a foreground program.\n");
...
void sigint_handler(int sig)
{
  ...
  if(!pid)
    unix_error("foreground doesn't have any process");

...
void eval(char *cmdline)
{
    char *argv[MAXARGS];    /* parsed command line */
    int  bg;                /* should child run as bg job? */
    pid_t pid;
    pid_t fg_pid;           /* pid of foreground job (if any) */
    sigset_t set;

    /* parse cmdline using the given function parseline */
    bg = parseline(cmdline, argv);

    /* do nothing when no token generated */
    if (argv[0] == NULL) /* array argv is null-terminated */
        return;
...
void do_bgfg(char **argv)
{
    ...
    /* 0 (no such job)
     * 1 (no such process)
     * 2 (input format is incorrect)
     * 3 (no input) */
    int  id;
    int  is_jobid;          /* 1 (id is jid), 0 (id is pid) */
    int  bg;                /* 1 (argv[0] is 'bg'), 0 (argv[0] is 'fg') */
    struct job_t *job;
    char *buf = argv[1];
    ...
    do
    {
        ...
        /* is_jobid - if argv[1] argument starts with % */
        is_jobid = (*buf == '%');

        /* ignore '%' when parsing string to integer */
        if (is_jobid)
            buf++;
        ...
        if (!isdigit(buf[0]))
        {
            error_code = 2; break;  /* parse error */
        }
        id = atoi(buf);
